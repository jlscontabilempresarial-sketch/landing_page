...
const YOUTUBE_VIDEO_ID = "HZjm50Au6Ec";
...
export default function Home() {
  ...
  return (
    <>
            ...
            >
              <div className="aspect-square bg-white/5 rounded-sm overflow-hidden relative group">
                <img 
                  ...
                  className="w-full h-full object-contain p-8 group-hover:p-4 transition-all duration-700 opacity-90 group-hover:opacity-100 scale-105 group-hover:scale-110"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-primary-deep/40 via-transparent to-transparent pointer-events-none" />
              </div>
      ...
      {/* Desenquadramento MEI Promo */}
      <section id="mei-migracao" className="py-20 relative overflow-hidden bg-white/[0.01]">
        <div className="absolute inset-0 bg-gradient-to-r from-accent-gold-dark/5 to-transparent" />
        <div className="container mx-auto px-6 relative z-10">
          <div className="bg-white/[0.02] border border-white/5 p-8 md:p-16 rounded-sm flex flex-col lg:flex-row items-center gap-12">
            <div className="flex-1">
              <span className="text-accent-gold-light text-sm font-semibold uppercase tracking-[3px] block mb-4">Crescimento & Regularização</span>
              <h2 className="text-3xl md:text-5xl font-bold mb-6 leading-tight">
                Sua empresa cresceu ou foi <span className="text-accent-gold-light">desenquadrada do MEI?</span>
              </h2>
              <p className="text-text-dim text-lg mb-8 max-w-xl">
                Seja pelo excesso de faturamento ou por desenquadramento de ofício, nós cuidamos de toda a migração e regularização da sua nova Microempresa.
              </p>
              <div className="flex flex-col sm:flex-row gap-4">
                <GoldButton to="/desenquadramento-mei" className="px-10">
                  Ver Plano de Migração
                </GoldButton>
                <div className="flex items-center gap-3 px-6 py-4 bg-white/5 rounded-[4px] border border-white/10">
                   <div className="w-2 h-2 rounded-full bg-accent-gold-light animate-pulse" />
                   <p className="text-xs font-bold uppercase tracking-widest">Consultoria Especializada</p>
                </div>
              </div>
            </div>
            <div className="hidden lg:grid flex-shrink-0 grid-cols-2 gap-4 w-full lg:w-auto">
              {[
                { label: "Segurança Fiscal", icon: <CheckCircle2 className="text-accent-gold-light" size={20} /> },
                { label: "Migração Rápida", icon: <CheckCircle2 className="text-accent-gold-light" size={20} /> },
                { label: "Gestão para ME", icon: <CheckCircle2 className="text-accent-gold-light" size={20} /> },
                { label: "Zero Multas", icon: <CheckCircle2 className="text-accent-gold-light" size={20} /> },
              ].map((item, i) => (
                <div key={i} className="flex items-center gap-3 p-4 bg-primary-deep border border-white/5 rounded-sm">
                  {item.icon}
                  <span className="text-xs font-bold uppercase tracking-wider">{item.label}</span>
